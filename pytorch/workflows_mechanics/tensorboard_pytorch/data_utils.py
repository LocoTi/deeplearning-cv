...
def generate_label_file(root):
    ...
    # 存储的相对路径，如: daisy/5547758_eea9edfd54_n.jpg, roses/12240303_80d87f77a3_n.jpg
    images_path = []  # 存储所有图片路径
    images_label = []  # 存储图片对应类别
    supported = [".jpg", ".JPG", ".png", ".PNG"]  # 支持的文件后缀类型
    # 遍历每个文件夹下的文件
    for cla in flower_class:
        cla_path = os.path.join(root, cla)
        ...
        for img_path in images:
            ...
            images_label.append(cla)

    label_path = os.path.join(root, "label.txt")
    with open(label_path, "w") as f:
        for image_name, class_name in zip(images_path, images_label):
            f.write(image_name + " " + class_name + "\n")
    
    print("write {} file finished.".format(label_path))
